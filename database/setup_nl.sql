...
COMMENT ON TABLE cravesaver.dishes IS 'A restaurant food delivery menu. Use this to find dishes matching user constraints on price, dietary_type, delivery_time, and descriptive subjective moods. Wait time is the est_delivery_mins column. Diet is dietary_type.';
...
-- 4. Teach the model exactly how to combine math and AI via a few-shot prescriptive example.
-- We can actually insert prescriptive rules directly into the configuration.
SELECT alloydb_ai_nl.add_prescriptive_rule(
   'For queries mentioning subjective feelings, vibes, descriptions or moods (e.g., "I feel sad", "comfort food", "healthy post-workout"), always sort using an inner vector distance match against mood_embedding: ORDER BY mood_embedding <=> embedding(''textembedding-gecko@003'', :human_prompt)::vector ASC.'
);
